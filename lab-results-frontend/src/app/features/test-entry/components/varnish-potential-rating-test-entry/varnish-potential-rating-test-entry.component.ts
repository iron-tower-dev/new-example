...
@Component({
    selector: 'app-varnish-potential-rating-test-entry',
    standalone: true,
    imports: [SharedModule],
    template: `
        <div class="varnish-potential-rating-test-container">
            <!-- Header Section -->
            <mat-card class="header-card">
                <mat-card-header>
                    <mat-card-title>Varnish Potential Rating Test Entry</mat-card-title>
                    <mat-card-subtitle>
            ...
            <!-- Main Form -->
            @if (varnishPotentialRatingForm && !isLoading()) {
                <form [formGroup]="varnishPotentialRatingForm" (ngSubmit)="onSave()">
                    ...
                    <mat-card class="trials-card">
                        <mat-card-header>
                            ...
                            <mat-card-subtitle>Enter varnish potential rating for each trial</mat-card-subtitle>
                        </mat-card-header>
                        <mat-card-content>
                                    ...
                                    <div class="trial-row" [formGroupName]="$index">
                                        ...
                                        <div class="trial-fields">
                                            <mat-form-field appearance="outline">
                                                ...
                                                <mat-hint>Varnish potential rating value</mat-hint>
                                                @if (getTrialControl($index, 'varnishPotentialRating')?.errors?.['required']) {
                                                    <mat-error>Varnish potential rating is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'varnishPotentialRating')?.errors?.['min']) {
                                                    <mat-error>Varnish potential rating must be 0 or greater</mat-error>
                                                }
                                            </mat-form-field>
                                        </div>
                                    </div>
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!varnishPotentialRatingForm.valid || isLoading()">
            ...
            @if (testResultHistory().length > 0) {
                <mat-card class="history-card">
                    <mat-card-header>
                        <mat-card-title>Last 12 Results for Varnish Potential Rating Test</mat-card-title>
                    </mat-card-header>
                    <mat-card-content>
                        <div class="history-table-container">
                            <table mat-table [dataSource]="testResultHistory()" class="history-table">
                                ...
                                <ng-container matColumnDef="results">
                                    <th mat-header-cell *matHeaderCellDef>VPR Values</th>
                                    <td mat-cell *matCellDef="let result">
                                        @for (trial of result.trials; track trial.trialNumber) {
                                            @if (trial.values.varnishPotentialRating) {
                                                <span class="trial-result">
                                                    T{{ trial.trialNumber }}: {{ trial.values.varnishPotentialRating | number:'1.1-1' }}
                                                </span>
                                            }
                                        }
                                    </td>
    ...
    `,
    styles: [`
        .varnish-potential-rating-test-container {
            padding: 20px;
            max-width: 1200px;
            margin: 0 auto;
        }
...
export class VarnishPotentialRatingTestEntryComponent implements OnInit, OnDestroy {
    ...
    // Component state
    varnishPotentialRatingForm!: FormGroup;
    testId = 286; // Varnish Potential Rating test ID - matches the seeded database
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'results'];

    // Form getters
    get trialsArray(): FormArray {
        return this.varnishPotentialRatingForm.get('trials') as FormArray;
    }
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (sample && this.varnishPotentialRatingForm) {
        ...
        effect(() => {
            ...
            if (template && !this.varnishPotentialRatingForm) {
                this.initializeForm();
            }
        });
    }
    ...
    private loadExistingResults(sampleId: number): void {
        this.testService.getTestResults(this.testId, sampleId).subscribe({
            next: (result) => {
                if (result && this.varnishPotentialRatingForm) {
    ...
    private initializeForm(): void {
        this.varnishPotentialRatingForm = this.fb.group({
    ...
    private addTrial(): void {
        const trialGroup = this.fb.group({
            varnishPotentialRating: [null, [Validators.required, Validators.min(0)]],
            isComplete: [false]
        });
    ...
    private populateFormWithResults(result: any): void {
        ...
        result.trials.forEach((trial: any) => {
            const trialGroup = this.fb.group({
                varnishPotentialRating: [trial.values.varnishPotentialRating || null, [Validators.required, Validators.min(0)]],
        ...
        // Set comments
        this.varnishPotentialRatingForm.patchValue({
    ...
    onTrialCompleteChange(trialIndex: number): void {
        ...
        if (isComplete) {
            ...
            const varnishPotentialRating = trialGroup.get('varnishPotentialRating')?.value;

            if (varnishPotentialRating === null || varnishPotentialRating === undefined) {
    ...
    onSave(): void {
        if (!this.varnishPotentialRatingForm.valid) {
            this.markFormGroupTouched(this.varnishPotentialRatingForm);
        ...
        const formValue = this.varnishPotentialRatingForm.value;
        const request: SaveTestResultRequest = {
            ...
            trials: formValue.trials.map((trial: any, index: number) => ({
                trialNumber: index + 1,
                values: {
                    varnishPotentialRating: trial.varnishPotentialRating
                },
    ...
    onClear(): void {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.varnishPotentialRatingForm.reset();
                this.initializeForm();
