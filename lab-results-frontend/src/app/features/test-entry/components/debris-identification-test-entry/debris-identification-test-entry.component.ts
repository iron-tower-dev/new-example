...
interface DebrisIdentificationResult {
    ...
    customVolume: string | null;
}

@Component({
    selector: 'app-debris-identification-test-entry',
    standalone: true,
    imports: [SharedModule, ParticleAnalysisCardComponent],
    template: `
        <div class="debris-identification-container">
            <!-- Header Section -->
            <mat-card class="header-card">
                <mat-card-header>
                    <mat-card-title>Debris Identification Test Entry</mat-card-title>
                    <mat-card-subtitle>
            ...
            <!-- Main Form -->
            @if (debrisIdentificationForm && !isLoading()) {
                <form [formGroup]="debrisIdentificationForm" (ngSubmit)="onSave()">
                    ...
                    <!-- Volume of Oil Used Card -->
                    <mat-card class="volume-card">
                        <mat-card-header>
                            <mat-card-title>Volume of Oil Used</mat-card-title>
                            <mat-card-subtitle>Select the volume of oil used for analysis</mat-card-subtitle>
                        </mat-card-header>
                        <mat-card-content>
                            <mat-radio-group formControlName="volumeOfOilUsed" class="volume-options">
                                ...
                                <mat-radio-button value="custom">Approx. X ml (Custom)</mat-radio-button>
                            </mat-radio-group>

                            @if (debrisIdentificationForm.get('volumeOfOilUsed')?.value === 'custom') {
                                <mat-form-field appearance="outline" class="custom-volume-field">
                                    <mat-label>Custom Volume (ml)</mat-label>
                                    <input 
                                        matInput 
                                        type="text" 
                                        formControlName="customVolume"
                                        placeholder="Enter volume in ml">
                                    <mat-hint>Enter approximate volume used</mat-hint>
                                    @if (debrisIdentificationForm.get('customVolume')?.hasError('required')) {
                                        <mat-error>Please enter a custom volume</mat-error>
                                    }
                                </mat-form-field>
                            }
                            ...
                            @if (selectedVolume()) {
                                <div class="volume-display">
                                    ...
                                    <strong>Selected Volume: {{ selectedVolume() }}</strong>
                                </div>
                            }
                        </mat-card-content>
                    </mat-card>
                        ...
                        <mat-card-content>
                            <mat-form-field appearance="outline" class="full-width">
                                ...
                                <textarea 
                                    ...
                                    rows="4"
                                    placeholder="Enter overall test findings, debris identification details, and recommendations...">
                                </textarea>
                                <mat-hint>Provide a comprehensive summary of the debris identification analysis</mat-hint>
                            </mat-form-field>
                        </mat-card-content>
                    </mat-card>
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!debrisIdentificationForm.valid || isLoading()">
                                ...
                                <div class="history-table-container">
                                    <table mat-table [dataSource]="testResultHistory()" class="history-table">
                                        ...
                                        <ng-container matColumnDef="volume">
                                            ...
                                            <td mat-cell *matCellDef="let result">{{ result.volumeOfOilUsed }}</td>
                                        </ng-container>
    ...
    `,
    styles: [`
        .debris-identification-container {
            padding: 20px;
            max-width: 1400px;
            margin: 0 auto;
        }

        .header-card, .sample-info-card, .filter-controls-card, .volume-card, .narrative-card, .history-card {
            margin-bottom: 20px;
        }
...
export class DebrisIdentificationTestEntryComponent implements OnInit, OnDestroy {
    ...
    readonly selectedVolume = computed(() => {
        if (!this.debrisIdentificationForm) return null;
        const volume = this.debrisIdentificationForm.get('volumeOfOilUsed')?.value;
        const custom = this.debrisIdentificationForm.get('customVolume')?.value;
        if (volume === 'custom' && custom) {
            return `Approx. ${custom} ml`;
        }
        return volume && volume !== 'custom' ? volume : null;
    });

    // Component state
    debrisIdentificationForm!: FormGroup;
    testId = 240; // Debris Identification test ID
    viewFilter: 'all' | 'review' = 'all';
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'severity', 'volume'];
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (sample && this.debrisIdentificationForm) {
    ...
    private loadExistingResults(sampleId: number): void {
        this.testService.getDebrisIdentificationResults(sampleId, this.testId).subscribe({
            next: (result) => {
                if (result && this.debrisIdentificationForm) {
    ...
    private initializeForm(): void {
        this.debrisIdentificationForm = this.fb.group({
            volumeOfOilUsed: ['', Validators.required],
            customVolume: [''],
            narrative: ['']
        });

        // Watch for volume selection changes to add/remove custom volume validation
        this.debrisIdentificationForm.get('volumeOfOilUsed')?.valueChanges
            .pipe(takeUntil(this.destroy$))
            .subscribe(value => {
                const customVolumeControl = this.debrisIdentificationForm.get('customVolume');
                if (value === 'custom') {
                    customVolumeControl?.setValidators([Validators.required]);
                } else {
                    customVolumeControl?.clearValidators();
                    customVolumeControl?.setValue('');
                }
                customVolumeControl?.updateValueAndValidity();
            });
    }

    private populateFormWithResults(result: any): void {
        // Set form values
        this.debrisIdentificationForm.patchValue({
            volumeOfOilUsed: result.volumeOfOilUsed || '',
            customVolume: result.customVolume || '',
     ...
     */
    onSave(): void {
        if (!this.debrisIdentificationForm.valid) {
        ...
        const formValue = this.debrisIdentificationForm.getRawValue();
        const result: DebrisIdentificationResult = {
            sampleId: sample.id,
            testId: this.testId,
            ...
            volumeOfOilUsed: formValue.volumeOfOilUsed,
            customVolume: formValue.volumeOfOilUsed === 'custom' ? formValue.customVolume : null
        };
     ...
     */
    onCancel(): void {
        if (this.debrisIdentificationForm.dirty) {
            const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
                data: {
