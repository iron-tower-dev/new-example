...
@Component({
    selector: 'app-tan-test-entry',
    standalone: true,
    imports: [SharedModule],
    template: `
        <div class="tan-test-container">
            <!-- Header Section -->
            <mat-card class="header-card">
                <mat-card-header>
                    <mat-card-title>TAN by Color Indication Test Entry</mat-card-title>
                    <mat-card-subtitle>
            ...
            <!-- Main Form -->
            @if (tanForm && !isLoading()) {
                <form [formGroup]="tanForm" (ngSubmit)="onSave()">
                    ...
                    <mat-card class="trials-card">
                        <mat-card-header>
                            ...
                            <mat-card-subtitle>Enter sample weight and final buret readings for each trial</mat-card-subtitle>
                        </mat-card-header>
                        <mat-card-content>
                                    ...
                                    <div class="trial-row" [formGroupName]="$index">
                                        ...
                                        <div class="trial-fields">
                                            <mat-form-field appearance="outline">
                                                ...
                                                @if (getTrialControl($index, 'sampleWeight')?.errors?.['required']) {
                                                    <mat-error>Sample weight is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'sampleWeight')?.errors?.['min']) {
                                                    <mat-error>Sample weight must be at least 0.01g</mat-error>
                                                }
                                            </mat-form-field>
                                            ...
                                            <mat-form-field appearance="outline">
                                                ...
                                                <mat-hint>Final buret reading in milliliters</mat-hint>
                                                @if (getTrialControl($index, 'finalBuret')?.errors?.['required']) {
                                                    <mat-error>Final buret reading is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'finalBuret')?.errors?.['min']) {
                                                    <mat-error>Final buret reading must be 0 or greater</mat-error>
                                                }
                                            </mat-form-field>
                                            ...
                                            <mat-form-field appearance="outline">
                                                ...
                                                <mat-hint>Calculated: (Final Buret × 5.61) ÷ Sample Weight</mat-hint>
                                            </mat-form-field>
                                        </div>
                                    </div>
                                }
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!tanForm.valid || isLoading()">
            ...
            @if (testResultHistory().length > 0) {
                <mat-card class="history-card">
                    <mat-card-header>
                        <mat-card-title>Last 12 Results for TAN Test</mat-card-title>
                    </mat-card-header>
                    <mat-card-content>
...
export class TanTestEntryComponent implements OnInit, OnDestroy {
    ...
    // Component state
    tanForm!: FormGroup;
    testId = 1; // TAN test ID - this should come from route or configuration
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'results'];

    // Form getters
    get trialsArray(): FormArray {
        return this.tanForm.get('trials') as FormArray;
    }
    ...
    private addTrial(): void {
        const trialGroup = this.fb.group({
            sampleWeight: [null, [Validators.required, Validators.min(0.01)]],
            finalBuret: [null, [Validators.required, Validators.min(0)]],
            tanResult: [{ value: null, disabled: true }],
            isComplete: [false]
        });
    ...
    private populateFormWithResults(result: any): void {
        ...
        result.trials.forEach((trial: any) => {
            const trialGroup = this.fb.group({
                sampleWeight: [trial.values.sampleWeight || null, [Validators.required, Validators.min(0.01)]],
                finalBuret: [trial.values.finalBuret || null, [Validators.required, Validators.min(0)]],
                tanResult: [{ value: trial.calculatedResult || null, disabled: true }],
    ...
    onTrialValueChange(trialIndex: number): void {
        ...
        const sampleWeight = trialGroup.get('sampleWeight')?.value;
        const finalBuret = trialGroup.get('finalBuret')?.value;

        if (sampleWeight && finalBuret && sampleWeight > 0) {
            // Calculate TAN: (Final Buret * 5.61) / Sample Weight
            let tanResult = (finalBuret * 5.61) / sampleWeight;

            // Apply minimum value of 0.01
            if (tanResult < 0.01) {
                tanResult = 0.01;
            }

            // Round to 2 decimal places
            tanResult = Math.round(tanResult * 100) / 100;

            trialGroup.get('tanResult')?.setValue(tanResult);
        } else {
            trialGroup.get('tanResult')?.setValue(null);
        }
    }
    ...
    onSave(): void {
        if (!this.tanForm.valid) {
            this.markFormGroupTouched(this.tanForm);
        ...
        const request: SaveTestResultRequest = {
            ...
            trials: formValue.trials.map((trial: any, index: number) => ({
                trialNumber: index + 1,
                values: {
                    sampleWeight: trial.sampleWeight,
                    finalBuret: trial.finalBuret
                },
                calculatedResult: trial.tanResult,
