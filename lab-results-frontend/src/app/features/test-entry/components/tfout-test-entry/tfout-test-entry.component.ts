...
@Component({
    selector: 'app-tfout-test-entry',
    standalone: true,
    imports: [SharedModule],
    template: `
        <div class="tfout-test-container">
            <!-- Header Section -->
            <mat-card class="header-card">
                <mat-card-header>
                    <mat-card-title>TFOUT Test Entry</mat-card-title>
                    <mat-card-subtitle>
            ...
            <!-- Main Form -->
            @if (tfoutForm && !isLoading()) {
                <form [formGroup]="tfoutForm" (ngSubmit)="onSave()">
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!tfoutForm.valid || isLoading()">
            ...
            @if (testResultHistory().length > 0) {
                <mat-card class="history-card">
                    <mat-card-header>
                        <mat-card-title>Last 12 Results for TFOUT Test</mat-card-title>
                    </mat-card-header>
                    <mat-card-content>
...
export class TfoutTestEntryComponent implements OnInit, OnDestroy {
    ...
    // Component state
    tfoutForm!: FormGroup;
    testId = 230; // TFOUT test ID - matches the seeded database
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'results'];

    // Form getters
    get trialsArray(): FormArray {
        return this.tfoutForm.get('trials') as FormArray;
    }
    ...
    onSave(): void {
        if (!this.tfoutForm.valid) {
            this.markFormGroupTouched(this.tfoutForm);
        ...
        const formValue = this.tfoutForm.value;
