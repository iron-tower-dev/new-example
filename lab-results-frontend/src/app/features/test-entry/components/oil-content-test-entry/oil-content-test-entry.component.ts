...
@Component({
    selector: 'app-oil-content-test-entry',
    standalone: true,
    imports: [SharedModule],
    template: `
        <div class="oil-content-test-container">
            <!-- Header Section -->
            <mat-card class="header-card">
                <mat-card-header>
                    <mat-card-title>Oil Content Test Entry</mat-card-title>
                    <mat-card-subtitle>
            ...
            <!-- Main Form -->
            @if (oilContentForm && !isLoading()) {
                <form [formGroup]="oilContentForm" (ngSubmit)="onSave()">
                    ...
                    <mat-card class="trials-card">
                        <mat-card-header>
                            ...
                            <mat-card-subtitle>Enter oil content percentage for each trial</mat-card-subtitle>
                        </mat-card-header>
                        <mat-card-content>
                                        ...
                                        <div class="trial-fields">
                                            <mat-form-field appearance="outline">
                                                ...
                                                <input 
                                                    ...
                                                    min="0"
                                                    max="100"
                                                    formControlName="oilContentPercent"
                                                    placeholder="Enter oil content percentage">
                                                <mat-hint>Oil content as percentage (0-100%)</mat-hint>
                                                @if (getTrialControl($index, 'oilContentPercent')?.errors?.['required']) {
                                                    <mat-error>Oil content percentage is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'oilContentPercent')?.errors?.['min']) {
                                                    <mat-error>Oil content must be 0% or greater</mat-error>
                                                }
                                                @if (getTrialControl($index, 'oilContentPercent')?.errors?.['max']) {
                                                    <mat-error>Oil content must be 100% or less</mat-error>
                                                }
                                            </mat-form-field>
                                        </div>
                                    </div>
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!oilContentForm.valid || isLoading()">
            ...
            @if (testResultHistory().length > 0) {
                <mat-card class="history-card">
                    <mat-card-header>
                        <mat-card-title>Last 12 Results for Oil Content Test</mat-card-title>
                    </mat-card-header>
                    <mat-card-content>
                        <div class="history-table-container">
                            <table mat-table [dataSource]="testResultHistory()" class="history-table">
                                ...
                                <ng-container matColumnDef="results">
                                    <th mat-header-cell *matHeaderCellDef>Oil Content (%)</th>
                                    <td mat-cell *matCellDef="let result">
                                        @for (trial of result.trials; track trial.trialNumber) {
                                            @if (trial.values.oilContentPercent) {
                                                <span class="trial-result">
                                                    T{{ trial.trialNumber }}: {{ trial.values.oilContentPercent | number:'1.2-2' }}%
                                                </span>
                                            }
                                        }
                                    </td>
...
export class OilContentTestEntryComponent implements OnInit, OnDestroy {
    ...
    // Component state
    oilContentForm!: FormGroup;
    testId = 285; // Oil Content test ID - matches the seeded database
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'results'];

    // Form getters
    get trialsArray(): FormArray {
        return this.oilContentForm.get('trials') as FormArray;
    }
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (template && !this.oilContentForm) {
                this.initializeForm();
            }
        });
    }
    ...
    private initializeForm(): void {
        this.oilContentForm = this.fb.group({
    ...
    private addTrial(): void {
        const trialGroup = this.fb.group({
            oilContentPercent: [null, [Validators.required, Validators.min(0), Validators.max(100)]],
            isComplete: [false]
        });
    ...
    private populateFormWithResults(result: any): void {
        ...
        result.trials.forEach((trial: any) => {
            const trialGroup = this.fb.group({
                oilContentPercent: [trial.values.oilContentPercent || null, [Validators.required, Validators.min(0), Validators.max(100)]],
        ...
        // Set comments
        this.oilContentForm.patchValue({
    ...
    onTrialCompleteChange(trialIndex: number): void {
        ...
        if (isComplete) {
            ...
            const oilContentPercent = trialGroup.get('oilContentPercent')?.value;

            if (oilContentPercent === null || oilContentPercent === undefined) {
    ...
    onSave(): void {
        if (!this.oilContentForm.valid) {
            this.markFormGroupTouched(this.oilContentForm);
        ...
        const formValue = this.oilContentForm.value;
        const request: SaveTestResultRequest = {
            ...
            trials: formValue.trials.map((trial: any, index: number) => ({
                trialNumber: index + 1,
                values: {
                    oilContentPercent: trial.oilContentPercent
                },
