...
@Component({
    selector: 'app-deleterious-test-entry',
    standalone: true,
    imports: [SharedModule],
    template: `
        <div class="deleterious-test-container">
            <!-- Header Section -->
            ...
            <!-- Main Form -->
            @if (deleteriousForm && !isLoading()) {
                <form [formGroup]="deleteriousForm" (ngSubmit)="onSave()">
                    ...
                    <mat-card class="trials-card">
                        <mat-card-header>
                            ...
                            <mat-card-subtitle>Enter MTE selection, pressure, scratches, and pass/fail result for each trial</mat-card-subtitle>
                        </mat-card-header>
                        <mat-card-content>
                                    ...
                                    <div class="trial-row" [formGroupName]="$index">
                                        ...
                                        <div class="trial-fields">
                                            <app-equipment-selection
                                                label="MTE Equipment"
                                                placeholder="Select MTE equipment"
                                                equipmentType="general"
                                                [required]="true"
                                                [showCalibrationHint]="false"
                                                (equipmentSelected)="onEquipmentSelected($index, $event)">
                                            </app-equipment-selection>
                                            ...
                                            <mat-form-field appearance="outline">
                                                ...
                                                <mat-hint>Pressure measurement</mat-hint>
                                                @if (getTrialControl($index, 'pressure')?.errors?.['required']) {
                                                    <mat-error>Pressure is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'pressure')?.errors?.['min']) {
                                                    <mat-error>Pressure must be 0 or greater</mat-error>
                                                }
                                            </mat-form-field>
                                            ...
                                            <mat-form-field appearance="outline">
                                                ...
                                                <mat-hint>Number of scratches observed</mat-hint>
                                                @if (getTrialControl($index, 'scratches')?.errors?.['required']) {
                                                    <mat-error>Number of scratches is required</mat-error>
                                                }
                                                @if (getTrialControl($index, 'scratches')?.errors?.['min']) {
                                                    <mat-error>Number of scratches must be 0 or greater</mat-error>
                                                }
                                            </mat-form-field>

                                            <div class="pass-fail-section">
                                                <label class="result-label">Test Result</label>
                                                <mat-radio-group 
                                                    formControlName="testResult"
                                                    class="pass-fail-group">
                                                    <mat-radio-button value="Pass" class="result-option pass">
                                                        Pass
                                                    </mat-radio-button>
                                                    <mat-radio-button value="Fail" class="result-option fail">
                                                        Fail
                                                    </mat-radio-button>
                                                </mat-radio-group>
                                                @if (getTrialControl($index, 'testResult')?.errors?.['required']) {
                                                    <mat-error class="result-error">Test result is required</mat-error>
                                                }
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="action-buttons">
                        <button 
                            ...
                            color="primary" 
                            type="submit"
                            [disabled]="!deleteriousForm.valid || isLoading()">
            ...
            @if (testResultHistory().length > 0) {
                <mat-card class="history-card">
                    <mat-card-header>
                        <mat-card-title>Last 12 Results for Deleterious Test</mat-card-title>
                    </mat-card-header>
                    <mat-card-content>
                                ...
                                <ng-container matColumnDef="results">
                                    ...
                                    <td mat-cell *matCellDef="let result">
                                        @for (trial of result.trials; track trial.trialNumber) {
                                            @if (trial.values.testResult) {
                                                <span class="trial-result" [class]="getResultClass(trial.values.testResult)">
                                                    T{{ trial.trialNumber }}: {{ trial.values.testResult }}
                                                    @if (trial.values.pressure) {
                                                        (P: {{ trial.values.pressure }}, S: {{ trial.values.scratches }})
                                                    }
                                                </span>
                                            }
                                        }
    ...
    styles: [`
        ...
        .result-option.pass ::ng-deep .mat-radio-label-content {
            color: #4caf50;
            font-weight: 500;
        }

        .result-option.fail ::ng-deep .mat-radio-label-content {
            color: #f44336;
            font-weight: 500;
        }
...
export class DeleteriousTestEntryComponent implements OnInit, OnDestroy {
    ...
    testId = 250; // Deleterious test ID - matches the seeded database
    historyDisplayedColumns = ['sampleId', 'entryDate', 'status', 'results'];

    // Form getters
    get trialsArray(): FormArray {
        return this.deleteriousForm.get('trials') as FormArray;
    }
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (sample && this.deleteriousForm) {
        ...
        effect(() => {
            ...
            if (template && !this.deleteriousForm) {
                this.initializeForm();
            }
        });
    }
    ...
    private loadExistingResults(sampleId: number): void {
        this.testService.getTestResults(this.testId, sampleId).subscribe({
            next: (result) => {
                if (result && this.deleteriousForm) {
    ...
    private addTrial(): void {
        const trialGroup = this.fb.group({
            ...
            pressure: [null, [Validators.required, Validators.min(0)]],
            scratches: [null, [Validators.required, Validators.min(0)]],
    ...
    private populateFormWithResults(result: any): void {
        ...
        result.trials.forEach((trial: any) => {
            const trialGroup = this.fb.group({
                equipmentId: [trial.values.equipmentId || null, [Validators.required]],
                pressure: [trial.values.pressure || null, [Validators.required, Validators.min(0)]],
                scratches: [trial.values.scratches || null, [Validators.required, Validators.min(0)]],
                testResult: [trial.values.testResult || null, [Validators.required]],
    ...
    onEquipmentSelected(trialIndex: number, event: EquipmentSelectionEvent): void {
        const trialGroup = this.trialsArray.at(trialIndex);
        trialGroup.get('equipmentId')?.setValue(event.equipmentId);
    }
    ...
    onTrialCompleteChange(trialIndex: number): void {
        ...
        if (isComplete) {
            ...
            const equipmentId = trialGroup.get('equipmentId')?.value;
            const pressure = trialGroup.get('pressure')?.value;
            const scratches = trialGroup.get('scratches')?.value;
            const testResult = trialGroup.get('testResult')?.value;

            if (!equipmentId || pressure === null || scratches === null || !testResult) {
    ...
    onSave(): void {
        if (!this.deleteriousForm.valid) {
            this.markFormGroupTouched(this.deleteriousForm);
        ...
        const request: SaveTestResultRequest = {
            ...
            trials: formValue.trials.map((trial: any, index: number) => ({
                ...
                values: {
                    ...
                    scratches: trial.scratches,
                    testResult: trial.testResult
                },
