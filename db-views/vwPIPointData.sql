...
/****** Object:  View [dbo].[vwPIPointData]    Script Date: 10/28/2025 11:31:08 AM ******/
DROP VIEW [dbo].[vwPIPointData]
GO
...
CREATE VIEW [dbo].[vwPIPointData]
AS
SELECT     dbo.AllResults.SampleID,dbo.allsamplecomments.CommentArea,dbo.allsamplecomments.Comment,
	case
		when    dbo.allsamplecomments.Comment like 'Severity:=normal%'  then '1'
		when    dbo.allsamplecomments.Comment like 'Severity:=alert%'  then '2'
		When    dbo.allsamplecomments.Comment like 'Severity:=monitor%'  then '3'
		when    dbo.allsamplecomments.Comment like 'Severity:=na%'  then '4'
		when    dbo.allsamplecomments.Comment like 'Severity:=alarm%'  then '5'
		when    dbo.allsamplecomments.Comment like 'Severity:=not set%'  then '6'
		else '0'
	end as severity,
 dbo.AllResults.GroupName, dbo.AllResults.Lube, dbo.AllResults.EQID, dbo.AllResults.Comp, dbo.AllResults.Loc, 
...
FROM         dbo.Lube_Sampling_Point INNER JOIN
					  ...
					  INNER JOIN dbo.allsamplecomments on dbo.AllResults.SampleID =dbo.allsamplecomments.SampleID
WHERE     (dbo.Component.site_id = 1) AND (dbo.Location.site_id = 1)
and CommentArea = 'ENGP'
