...
'Response.Write "____AddTestToSchedule return value: " & (DBErrorCount(conn)=0) & "<br>"
  ...
  if DBErrorCount(conn)=0 then
    sql="UPDATE usedlubesamples SET status=90 WHERE status IN (80,120) AND ID=" & sampleid
...
' To revert status C to X for upper/lower limits violation, set TestReadings.entryId=NULL
' Kwaku 12/16/09
'
...
dim rs,conn,sql
  ...
  if not rs.EOF and (blnoverwrite or len(getField(rs,"schedule"))=0) then
    ...
    Err.Clear
    ' this section inserted by Kwaku to force vwLabOverview to display X for limit violation
    ' also set the entryId on TestReadings.entryId = NULL
    sql="update TestReadings set entryID=NULL WHERE id=" & sampleid
    set rs=conn.Execute(sql)
  end if
  CloseDBObject(rs)
  set rs=nothing
...
IsTestRequired = False		' assume false
...
'                  Response.Write "    Calling function to remove " & ruletestid & "<br>"
                  if RemoveTestFromSchedule(sampleid,ruletestid,"Test removed due to defined rules") then
                    ...
                    SetSampleScheduleType sampleid,"A",continue		' kwaku change 12/16/09
                  end if
                end if
              else
